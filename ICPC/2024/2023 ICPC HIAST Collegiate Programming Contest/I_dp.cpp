...
int query(int l, int r) { int t = p[r]; add(t, -query(l-1)); return t; }
...
void solve(int testID){
	...
	int kmax = max(n0/l0, n1/l1) + 2;
	// DBGn(kmax)
	p[0] = 1;
	for (int k = 1; k <= kmax; k++) {
		int mn = (k-1) * l0;
		for (int i = mn+1; i <= n0; i++) {
			p[i] = p[i-1];
			add(p[i], f0[i]);
		}
		mn += l0;
		for (int i = n0; i >= mn; i--) f0[i] = query(i-r0, i-l0);
		for (int i = mn-l0; i < mn; i++) f0[i] = p[i] = 0;
		p[mn] = f0[mn];
	...
	for (int k = 1; k <= kmax; k++) {
		int mn = (k-1) * l1;
		for (int i = mn+1; i <= n1; i++) {
			p[i] = p[i-1];
			add(p[i], f1[i]);
		}
		mn += l1;
		for (int i = n1; i >= mn; i--) f1[i] = query(i-r1, i-l1);
		for (int i = mn-l1; i < mn; i++) f1[i] = p[i] = 0;
		p[mn] = f1[mn];

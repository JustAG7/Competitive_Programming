...
int n;
bool check(int k){
	// cout << "working on: " << k << nl;
	vector<vector<int>> section(k + 1);
	for(int i = 0; i < n; i++) {
		int part = ((i+1)* k + n - 1) / n;
		section[part].pb(a[i]);
	}
	// for(auto x : section ) {
	// 	for(auto y : x) cout << y << ' ';
	...
	for(int i = 0; i < section.size() - 1; i++) {
		for(int j = i + 1; j < section.size(); j++) {
			int mx = 0, mn = M;
			for(int z = 0; z < section[i].size(); z++) {
				mx = max(mx, section[i][z]);
				mn = min(mn, section[j][z]);
			}
			if(mx >= mn) ok = false;
		}
	}
...
void solve(int testID){
	...
	for(int i = 2; i <= n; i++) {
		if(n % i == 0) if(check(i)) ans.pb(i);
	}
	if(!ans.size()) return cout << -1, void();
